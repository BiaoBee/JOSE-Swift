...
struct JWTClaimTests {
    @Test("RFC 7519 example JWT claims")
    func decodeRFCClaims() async throws {
        let claimsData = """
            {
              "iss": "joe",
              "exp": 1300819380,
    ...
    @Test("Decoding public JWT claims")
    func decode() async throws {
        ...
        #expect(claims.exp == Date(timeIntervalSince1970: 1_752_109_200))
        #expect(claims.nbf == Date(timeIntervalSince1970: 1_752_105_600))
        #expect(claims.iat == Date(timeIntervalSince1970: 1_752_105_600))
        #expect(claims.jti == "a1b2c3d4e5f6")
    }

    @Test("Decode private JWT claims")
    func decodeWithParivateClaims() async throws {
        let claimsData = """
            {
              "iss": "https://auth.example.com",
              "sub": "user_98765",
    ...
    func encode() async throws {
        ...
        var claims = JWTClaims(
            ...
            nbf: date,
            iat: date,
            privateClaims: ["custom1": "value1", "custom2": "value2"]
        )
        claims["custom3"] = "value3"
        ...
        #expect(
            decodedClaims as! NSDictionary == [
                "iss": "iss", "sub": "sub", "aud": "aud", "exp": numericDate, "nbf": numericDate, "iat": numericDate,
                "custom1": "value1", "custom2": "value2", "custom3": "value3",
            ]
        )
    }

    @Test("Claims description")
    func description() async throws {
        ...
        let jsonPresentation = try String(data: encoder.encode(claims), encoding: .utf8)!
